...
for shadow_curr in ["Full","Partial","No"]:
    for month_curr in ["February","April"]:
        curr_mean=calculate_mean_groundhog(groundhog_data,"Punsutawney Phil",
...
def get_shadow_predict(prediction,month,mean_or_std):
    return float(shadow_predictions.loc[(shadow_predictions.Month==month)&(shadow_predictions.Shadow==prediction),
                                        mean_or_std])
